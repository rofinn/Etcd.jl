...
@testset "Etcd" begin
    ...
    @testset "requests" begin
        @testset "HTTPError" begin
            err = HTTPError(Requests.get("http://localhost:$port/$version/bad"))
            @test isa(Etcd.response(err), Requests.Response)
            show_out = sprint(showerror, err)
            @test startswith(show_out, "HTTPError: 404 page not found")
        end

        @testset "EtcdError" begin
            try
                create(cli, "/mykey", "myvalue"; ttl=2)
                create(cli, "/mykey", "myvalue"; ttl=2)
            catch err
